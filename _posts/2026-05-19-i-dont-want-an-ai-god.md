...
The agent needs to do more than say "I can help with that". It has to know what I said I wanted to be doing, and tell me to keep at it even when it is hard or boring. This is the gym trainer for your mind. Jason Weeby calls his version a [helmet](https://www.linkedin.com/pulse/ai-make-me-dumber-i-let-so-made-helmet-jason-weeby-twyuc){:target="_blank"}: persistent instructions baked into the AI itself that make it scaffold your thinking with questions rather than hand you finished answers, flag when you are asking it to decide rather than inform, and nudge you to keep the [desirable difficulty](https://bjorklab.psych.ucla.edu/research/){:target="_blank"} that learning actually requires. The reason most of us would let the AI write the strategy is that strategy is hard, the deadline is tomorrow, and the chat window is right there. A productivity stack that lets you do the easy thing every time is just a treadmill, continually producing average work.
...
But tools like Cowork are not really finished products. They are an Excel for AI agents. Excel enabled a generation of bespoke spreadsheet-applications that ran real businesses for years and were maintainable by no one; Access databases and Lotus Notes did the same. A powerful generic tool, given to everyone, produces an explosion of bespoke things that quietly start running real work and then rot. Cowork has some of the right primitives — connectors, schedules, proper memory — but the same generality that makes those primitives powerful guarantees the explosion.
