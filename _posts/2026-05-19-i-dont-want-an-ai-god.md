...
But tools like Cowork are not really finished products. They are substrates — an Excel for AI agents. Excel enabled a generation of bespoke spreadsheet-applications that ran real businesses for years and were maintainable by no one; Access databases and Lotus Notes did the same. A powerful generic tool, given to everyone, produces an explosion of bespoke things that quietly become load-bearing and then decay. Cowork has some of the right primitives — connectors, schedules, proper memory — but a substrate is not a product, and giving everyone the substrate guarantees the explosion.
