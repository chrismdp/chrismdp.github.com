...
If I had to name the moment, it was late November 2025: Claude Opus 4.5 landed alongside Anthropic's long-running agents post, and things I had been grinding at manually started running on their own. A year earlier, around late 2024, Cursor's agent mode did the same thing for the IDE generation of tools. Both moments had the same shape: capability crossed a threshold, then the tooling caught up over the following weeks as the rest of us worked out what to do with it. If your sense of what AI coding can do is calibrated to before November 2025, it is out of date.
...
The honest recommendation for a competent developer starting today is: install [Claude Code](https://claude.com/claude-code){:target="_blank"} or Codex CLI, not Copilot. The choice between the two is largely personal preference now, both run the current frontier, and Pi[^pi] and the other serious harnesses work well with the latest Claude or Codex models. Every other combination feels a step or two behind. Cursor was the right answer in March 2025 and I recommended it then. By May 2025 I had moved to Claude Code and have stayed there, but a developer starting today could reasonably pick either and be in the right neighbourhood. The reason is the harness, not the model. GitHub Copilot with Opus underneath does not produce Claude Code's results, because the wrapper around every request matters at least as much as the model itself. How files are chunked, how context is selected, how the agent loop is structured, how CLAUDE.md is privileged over arbitrary docs, these are harness decisions and they compound.

That recommendation does not survive contact with every procurement department. Plenty of organisations are locked into Copilot seats they bought last year, compliance posture that makes new vendor onboarding a six-month exercise, or hosting requirements that rule out commercial CLIs entirely. If that is you, read what follows as "what to aim for", not "install this by Friday". The principles transfer: the harness matters more than the model, AGENTS.md and skill files work with any agent CLI, and the reset discipline is tool-agnostic. Getting anything through procurement is its own senior skill in 2026!

Open-source models can keep up if you work at them, but they need a clean codebase to shine. Expect to spend more time on the harness and less on the prompt than you would with Claude or Codex, and expect the feedback loop between "agent got it wrong" and "agent stops getting it wrong" to be longer. Worth the effort for teams with the skin and the patience for it, but not where I would tell someone new to start.

On top of that harness sit three practical pieces.

**Golden rules in AGENTS.md or CLAUDE.md.** These are the standing instructions the agent reads at the start of every session in a given folder. Claude Code, Codex CLI, and Cursor now read both filenames, so I keep one file and symlink the other and they all pick up the same rules. I keep the file short, specific, and continuously refined. After every meaningful change, I ask the agent to update it based on what it wished it had known at the start. A year in, this compounding effect is the single biggest lever I have for code quality.

**Skill files.** A [skill file](/skills-are-claude-codes-secret-weapon/) in `.claude/skills/` sits between a strict rule and a background doc. Rules in AGENTS.md or CLAUDE.md load at the start of every session and in theory are always followed — in practice adherence depends on the harness — but they still carry more weight than anything else you give the agent, which forces them to stay short. Docs under `docs/` can hold as much as you like but rarely get read when it matters. Skills get pulled in when the work calls them, and they can hold much more detail than a rule would. That sweet spot is where your coding conventions, API rules, and test standards belong, and Claude can update them when it learns something new. I am running nearly 70 of these now across my projects. A handful are public; you can see them at [airskills.ai/chrismdp](https://airskills.ai/chrismdp) if you want a starting shape to copy from.
...
The unfashionable answer is fundamentals. Automated tests, continuous delivery, infrastructure as code, linting, type checking, everything you were meant to have in 2015. Without them the agent has no way to halt on an obvious failure and you are back to approving diffs by eye, which is the job AI was meant to eliminate. Teams that kept their CI pipeline sharp are flying in 2026; teams that did not are stuck rubber-stamping whatever the agent last produced. If someone tells you the fundamentals have been obsoleted, they are selling you something.
...
The fourth is **premature relief**. If you have got through the first wave of IDE-based AI coding and your team is working in Claude Code or Codex, do not assume the adjustment is over. Cloud agents and autonomous orchestration are the wave that the late-2025 step-up made possible, and the survival strategy for each wave is different. The teams that thought "glad that's over" a year ago are often the ones struggling most now.
...
**Install Claude Code or Codex CLI** and run it in a repo you already know. Write a short AGENTS.md with the conventions of that repo, the things a new engineer would want to know on their first day. Ship one small change through it, read the diff, and note where it surprised you. That is your first skill file.
