...
<div class="bg-brand-white rounded-lg p-8 border border-brand-light-blue/20 mb-12">
  <p class="text-lg text-brand-black/80 mb-6 text-center" id="checkin-intro">
    Thanks for taking the time to <strong>check in</strong> on how things are going since the training.
  </p>

  <div class="rm-area-embed-training-checkin"></div>
</div>
...
(function() {
  ...
  if (company) {
    var name = company.replace(/-/g, ' ').replace(/\b\w/g, function(l) { return l.toUpperCase(); });
    var intro = document.getElementById('checkin-intro');
    intro.innerHTML = 'Thanks for taking the time to <strong>check in</strong> on how things are going since the ' + name + ' training.';
  }
})();
</script>
